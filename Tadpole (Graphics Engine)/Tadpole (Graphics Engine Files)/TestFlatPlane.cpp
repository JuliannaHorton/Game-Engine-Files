...
TestFlatPlane::TestFlatPlane()
	: ShaderBase(L"../DX Application/Sprite.hlsl") //having trouble makign my own hlsl file, so I'm just gonna use ColorSelected
{
...
void TestFlatPlane::SendCamMatrices(const Matrix& view, const Matrix& proj)
{
...
void TestFlatPlane::SendWorldandTexture(const Matrix& world, Texture* tex)
{
...
void TestFlatPlane::SendFogData(const float& start, const float& range, const Vect& color)
{
	Data_Fog wm;
	wm.FogStart = start;
	wm.FogRange = range;
...
void TestFlatPlane::SetToContext()
{
