...
Mirror::Mirror(FlatPlane* plane)
	: pPlane(plane), ReflectionMat(Matrix(IDENTITY))
{
	...
	hr = RenderAttorney::GetDevice()->CreateDepthStencilState(&mirrorDesc, &MarkMirrorDSS);
	assert(SUCCEEDED(hr));
	...
	hr = RenderAttorney::GetDevice()->CreateDepthStencilState(&drawReflectionDesc, &DrawReflectionDSS);
	assert(SUCCEEDED(hr));
	...
	hr = RenderAttorney::GetDevice()->CreateRasterizerState(&rd, &MirrorFrontFaceAsClockWiseRS);
	assert(SUCCEEDED(hr));
	...
	hr = RenderAttorney::GetDevice()->CreateBlendState(&bd, &TransparentBS);
	assert(SUCCEEDED(hr));
	...
	hr = RenderAttorney::GetDevice()->CreateBlendState(&bd, &NoWriteToRenderTargetBS);
	assert(SUCCEEDED(hr));
}

void Mirror::SetReflection(Matrix Location)
{
	Pos = Location * Matrix(TRANS, 0, 0, -0.1); // Slight offset from the 'wall' to avoid z-fighting..Keep for now for testing 

	// Create Reflection matrix for mirror plane.
	// Uses the mirror's normal N and position
	Vect N = Pos.get(ROW_1); // Mirror normal
	Vect p = Pos.get(ROW_3); // point on mirror
	float d = -p.dot(N);
...
Matrix Mirror::GetReflection()
{
	return ReflectionMat;
}

void Mirror::Step2(ShaderColorLight* shader)
{
	// BLEND STATE: Stop writing to the render target 
	RenderAttorney::GetContext()->OMSetBlendState(NoWriteToRenderTargetBS, nullptr, 0xffffffff);
	// STENCIL: Set up the stencil for marking ('1' for all pixels that passed the depth test. See comment at line 35)
	RenderAttorney::GetContext()->OMSetDepthStencilState(MarkMirrorDSS, 1);

	//Something here...

	// Render the mirror 
	shader->SendWorldAndMaterial(Pos, Colors::DarkBlue, Colors::DarkBlue, Vect(0, 0, 0, 0)); //Change this later keep as Light for initial testing 
	pPlane->Render();

	// STENCIL: stop using the stencil
	RenderAttorney::GetContext()->OMSetDepthStencilState(0, 0);
	// BLEND STATE: Return the blend state to normal (writing to render target)
	RenderAttorney::GetContext()->OMSetBlendState(0, nullptr, 0xffffffff);

	// WINDINGS: face winding will appear inside out after reflection. Switching to CW front facing
	RenderAttorney::GetContext()->RSSetState(MirrorFrontFaceAsClockWiseRS);
	// STENCIL: Use the stencil test (reference value 1) and only pass the test if the stencil already had a one present
	RenderAttorney::GetContext()->OMSetDepthStencilState(DrawReflectionDSS, 1);
}

void Mirror::Step4(ShaderColorLight* shader)
{
...
//		   (this assumes your mirror has a tint/texture to it. If perfectly 'clear' then this isn't needed)
	...
	RenderAttorney::GetContext()->OMSetBlendState(TransparentBS, nullptr, 0xffffffff);

	shader->SendWorldAndMaterial(Pos, Vect(0, .5, 1, .3f), Vect(0, .5, 1, .3f), Vect(0, .5, 1, .3f));
