#ifndef MODEL_H
#define MODEL_H

#include <d3d11.h>
#include "Vect.h"
#include "Align16.h"
#include "Matrix.h"

class MeshSeparator;

struct StandardVertex : public Align16
{
	Vect Pos;
	float u = 0;
	float v = 0;
	Vect normal;
	Vect Color;
	int meshNum = 0;		// new property

	void set(float _x, float _y, float _z,
		float _u = 0, float _v = 0,
		float _nx = 0, float _ny = 0, float _nz = 0,
		float _r = 1, float _g = 1, float _b = 1, int num = 0 )
	{
		Pos = Vect(_x, _y, _z);
		u = _u;
		v = _v;
		normal = Vect(_nx, _ny, _nz, 0);
		Color = Vect(_r, _g, _b);
		meshNum = num;
	}

	void set(float _x, float _y, float _z, float _u, float _v, const Vect& c, int num = 0)
	{
		Pos = Vect(_x, _y, _z);
		u = _u;
		v = _v;
		Color = c;
		meshNum = num;
	}

	void set(float _x, float _y, float _z, float _u, float _v, const Vect& n, const Vect& c, int num = 0)
	{
		Pos = Vect(_x, _y, _z);
		u = _u;
		v = _v;
		normal = n;
		Color = c;
		meshNum = num;
	}

	void set(float _x, float _y, float _z, const Vect& c, int num = 0)
	{
		Pos = Vect(_x, _y, _z);
		u = 0;
		v = 0;
		Color = c;
		meshNum = num;
	}

	void set(float _x, float _y, float _z, const Vect& n, const Vect& c, int num = 0)
	{
		Pos = Vect(_x, _y, _z);
		u = 0;
		v = 0;
		normal = n;
		Color = c;
		meshNum = num;
	}

	void set(const Vect& p, const Vect& c)
	{
		Pos = p;
		Color = c;
	}
};

struct TriangleByIndex
{
	unsigned int v0;
	unsigned int v1;
	unsigned int v2;

	void set(int _v0, int _v1, int _v2)
	{
		v0 = _v0;
		v1 = _v1;
		v2 = _v2;
	}
};

class Model
{
friend class ModelAttorney;
private:
	// Model information
	TriangleByIndex *pTriList;
	StandardVertex* pStdVerts;
	int numVerts;
	int numTris;

	MeshSeparator* meshes;

	// GPU connections
	ID3D11Device*			mDevice;
	ID3D11Buffer*           mpVertexBuffer;
	ID3D11Buffer*           mpIndexBuffer;

	Vect* VectList; //Not named ArrayList to annoy Andre
	Vect* AABBmin;
	Vect* AABBmax;

	Vect* center;
	float radius;

	void privLoadDataToGPU();
	void privLoadDataFromFile(const char * const _modelName, StandardVertex*& pVerts, int& nverts, TriangleByIndex*& ptlist, int& ntri);

	void SetToContext();
	void RenderMesh(int meshnum);

public:
	enum PreMadeModels {
		UnitBox,
		UnitBoxRepeatedTexture,
		UnitPyramid,
		UnitPyramidRepeatedTexture,
		UnitBoxSixFacesTexture,
		UnitSphere,
		UnitSphereRepeatedTexture
	};

	Model() = delete;
	Model(StandardVertex *pVerts, int nverts, TriangleByIndex *ptlist, int ntri);
	Model(PreMadeModels pm);
	Model(const char * const _modelName);

	virtual ~Model();

	void Render();

	// Get access
	int GetMeshCount();
	bool ValidMeshNum(int i);

	Vect& getCenter() const;
	float getRadius() const;

	Vect* getVectList(void) const; //Keep as "VectList" to annoy Andre
	int getVectNum() const;

	TriangleByIndex* getTriangleList(void) const
	{
		return pTriList;
	}
	int getTriNum() const
	{
		return numTris;
	};

	Vect& getMinAABB() const;
	Vect& getMaxAABB() const;
};



#endif MODEL_H